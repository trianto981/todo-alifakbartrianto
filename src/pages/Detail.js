...
import editIcon from '../gambar/todo-title-edit-button.svg'
...
const Detail = () => {
    ...
    const getList = async () =>{
        try{
            setLoading(true);
            const response = await API(`/todo-item?activity_group_id=${activityId}`);
    ...
    const Check = <img src={checkIcon} className="ms-3" />
    ...
    <div className='container py-5'>
            ...
            {loading ? <Loading/> : 
                list.length < 1 ?
                    <div className='col text-center'>
                        <img src={empty} alt='empty list' data-cy='todo-empty-state'></img>
                    </div>
            :
        ...
        {listModalShow && item ?
            ...
            : <ListModal show={listModalShow} confirm={addItem} hide={hideListModal} type='item'/>    
