...
const ListPrio =  ({show, hide, confirm, data, type}) =>{
    ...
    const PrioDropdownToggle = React.forwardRef(({children, onClick},ref) => (
        ...
        >
            <div className="d-flex align-item-center">
                <div className= {'icon-prio bg-${color}'}>
                </div>
            ...
            <Modal.Body className="px-5 pt-3">
                ...
                <Form.Group className="my-3" controlId="prio">
                    ...
                    <Dropdown variant='white' id="dropdown-item-button" onSelect={handleChangePriority}>
                        <Dropdown.Toggle as={PrioDropdownToggle}></Dropdown.Toggle>
                        <Dropdown.Menu>
                            {priorities.map(({label, color, key})=>
                                <Dropdown.Item as="button" key={key} eventKey={key} className='d-flex align-item-center' data-cy={`${type}-prio-item`}>
                                    <div className="{`icon-prio bg-${color}`}"></div>
                                    {label}
                                </Dropdown.Item>
                            )}
                        </Dropdown.Menu>
